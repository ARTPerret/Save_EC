...
func initialize_garbage() -> void:
	var dechet_data: Array = SaveManager.getElement("Missions", "Dechet_Bas")
...
func on_interact(player: Player) -> void:
	...
	if not dechet_data.has(false):
		if SaveManager.getElement("Quests", "S_dechet_campus") == "haut":
			SaveManager.setElement("Quests", {"S_dechet_campus": "all"})
			Dialogic.start("prop_trash", "book2")
			await Dialogic.timeline_ended
			SaveManager.save()
		else:
			SaveManager.setElement("Quests", {"S_dechet_campus": "bas"})
			Dialogic.start("prop_trash", "book4")
			await Dialogic.timeline_ended
			SaveManager.save()
